...
export function useGsapContext<T extends HTMLElement = HTMLDivElement>(
  setup: (ctx: { self: T; gsap: typeof gsap }) => void,
  deps: unknown[] = []
) {
  ...
  useEffect(() => {
    ...
    if (reduced()) {
      el.querySelectorAll<HTMLElement>('.gsap-init').forEach((n) => {
        n.style.opacity = '1';
...
export function useRevealOnScroll<T extends HTMLElement = HTMLDivElement>(
  ...
  return useGsapContext<T>(({ self }) => {
    const targets = self.querySelectorAll<HTMLElement>(selector);
