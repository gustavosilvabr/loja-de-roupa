...
describe('markup aplicado ao catálogo real', () => {
  ...
  it('todo produto do catálogo tem venda maior que o custo', () => {
    const produtos = resolveProducts(CATALOGO_VAZIO, pricing);
    const invertidos = produtos.filter((p) => (_nullishCoalesce(p.custo, () => ( 0))) > 0 && p.price <= (_nullishCoalesce(p.custo, () => ( 0))));
...
describe('olheiro do catálogo', () => {
  ...
  it('detecta produto novo e mudança de preço', () => {
    ...
    expect(_optionalChain([novo, 'optionalAccess', _ => _.tipo])).toBe('novo');
    expect(_optionalChain([novo, 'optionalAccess', _2 => _2.custoNovo])).toBe(90);

    const preco = mudancas.find(
      (m) => m.handle === 'camisa-flamengo-i-26-27-torcedor-rubro-negra' && m.tipo === 'preco'
    );
    expect(_optionalChain([preco, 'optionalAccess', _3 => _3.custoAntigo])).toBe(80);
    expect(_optionalChain([preco, 'optionalAccess', _4 => _4.custoNovo])).toBe(95);
  });
