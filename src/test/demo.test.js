 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { describe, expect, it } from 'vitest';
...
describe('avaliações de demonstração', () => {
  it('preenche nota, contagem e comentários', () => {
    ...
    for (const h of HANDLES) {
      ...
      expect(_optionalChain([p, 'access', _ => _.comentarios, 'optionalAccess', _2 => _2.length])).toBe(p.avaliacoes);
    }
  });

  it('marca toda avaliação gerada como demonstração', () => {
    const c = aplicarDemo(VAZIO, HANDLES);
    const todas = Object.values(c.products).flatMap((p) => _nullishCoalesce(p.comentarios, () => ( [])));
...
describe('remoção da demonstração', () => {
  it('limpa nota, contagem e comentários', () => {
    ...
    for (const h of HANDLES) {
      expect(_optionalChain([limpo, 'access', _3 => _3.products, 'access', _4 => _4[h], 'optionalAccess', _5 => _5.nota])).toBeUndefined();
      expect(_optionalChain([limpo, 'access', _6 => _6.products, 'access', _7 => _7[h], 'optionalAccess', _8 => _8.avaliacoes])).toBeUndefined();
    }
  });
