...
export type NivelLucro = 'prejuizo' | 'baixo' | 'moderado' | 'bom';

export interface ParametrosCusto {
  /** Quanto você paga no fornecedor. */
  custo: number;
  /** Quanto o cliente paga pela peça. */
  precoVenda: number;
  /** Custo do frete que a loja absorve (0 se o cliente paga). */
  fretePago: number;
  /** Percentual cobrado pelo gateway (ex.: 4.99). */
  taxaGateway: number;
  /** Valor fixo por transação do gateway (ex.: 0.40). */
  taxaFixa: number;
  /** Percentual de imposto sobre a venda. */
  imposto: number;
  /** Embalagem, etiqueta, custo operacional por peça. */
  outrosCustos: number;
}
...
export interface ResultadoLucro {
  ...
  imposto: number;
  outros: number;
  /** Tudo que sai do bolso. */
  custoTotal: number;
  /** O que sobra. */
  lucro: number;
  /** Lucro sobre o preço de venda — a margem que interessa. */
  margem: number;
  /** Lucro sobre o custo — o "quantas vezes" você multiplica. */
  markup: number;
  nivel: NivelLucro;
}

export interface Faixas {
  /** Margem a partir da qual o lucro é considerado bom. */
  boa: number;
  /** Margem a partir da qual o lucro é moderado. */
  moderada: number;
}

export const FAIXAS_PADRAO: Faixas = { boa: 40, moderada: 20 };

/** Aplica um markup percentual sobre o custo. 80 com 125% vira 180. */
export const aplicarMarkup = (custo: number, markupPercent: number) =>
  Math.round(custo * (1 + markupPercent / 100) * 100) / 100;

/** Quanto de markup é preciso para o preço sair no valor desejado. */
export const markupNecessario = (custo: number, precoDesejado: number) =>
...
export function precoParaMargem(
  p: Omit<ParametrosCusto, 'precoVenda'>,
  margemAlvo: number
): number {
...
export function classificar(margem: number, faixas: Faixas = FAIXAS_PADRAO): NivelLucro {
...
export const ROTULO_NIVEL: Record<NivelLucro, string> = {
  prejuizo: 'Prejuízo',
  baixo: 'Lucro baixo',
...
export const COR_NIVEL: Record<NivelLucro, { texto: string; fundo: string; ponto: string }> = {
...
export function veredito(r: ResultadoLucro, faixas: Faixas = FAIXAS_PADRAO): string {
  switch (r.nivel) {
    case 'prejuizo':
